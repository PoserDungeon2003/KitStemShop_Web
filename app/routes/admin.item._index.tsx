...
export default function AdminItem() {
  ...
  const mapKitId = useMemo(() => {
    return _.mapKeys(kits.data?.data, it => it.kitId);
  }, [kits.data?.data]);

  const datasource = useMemo(() => {
    return _(items.data?.data)
      .orderBy(it => it.istemId, "desc")
      .value();
  }, [items.data?.data]);
  ...
  const columns: TableProps<Item>['columns'] = [
    ...
    },
    {
      ...
      render: (id: number) => (
        <Tooltip placement="topLeft" title={mapKitId[id]?.kitName}>
          {mapKitId[id]?.kitName}
        </Tooltip>
      ),
    },
    ...
    },
    {
      ...
      render: (record: Item) => (
        <IoEye
          ...
          onClick={() => navigate(`/admin/item/${record.istemId}`)}
        />
      ),
    },
  ];
  return (
    <div>
      <Table loading={items.isLoading} columns={columns} dataSource={datasource}>
      </Table>
    </div>
  )
}